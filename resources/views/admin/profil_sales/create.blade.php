...
<div class="container mx-auto p-6">
    ...
    <div class="bg-white rounded-xl shadow-sm">
        <form action="{{ route('profil_sales.store') }}" method="POST" enctype="multipart/form-data" class="p-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="id_users" 
                            id="id_users" 
                            class="w-full px-4 py-2 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500 @error('id_users') border-red-500 @enderror"
                            required>
                        <option value="">Pilih User</option>
                        @foreach($users as $user)
                            <option value="{{ $user->id_users }}" {{ old('id_users') == $user->id_users ? 'selected' : '' }}>
                                {{ $user->email }}
                            </option>
                        @endforeach
                    </select>
                ...
                <!-- Tim Sales Field -->
                <div>
                    <label for="id_tim_sales" class="block text-sm font-medium text-gray-700 mb-2">
                        Tim Sales
                    </label>
                    <select name="id_tim_sales" 
                        ...
                        @foreach($timSales as $tim)
                            <option value="{{ $tim->id_tim_sales }}" {{ old('id_tim_sales') == $tim->id_tim_sales ? 'selected' : '' }}>
                                {{ $tim->nama_tim_sales }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="id_jabatan" 
                            id="id_jabatan" 
                            class="w-full px-4 py-2 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500 @error('id_jabatan') border-red-500 @enderror"
                            required>
