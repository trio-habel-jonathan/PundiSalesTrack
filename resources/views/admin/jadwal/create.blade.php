...
<div class="container mx-auto p-6">
    ...
    <div class="flex items-center justify-between mb-8">
        <div>
            <h1 class="text-2xl font-semibold">Tambah Jadwal</h1>
            <p class="text-sm text-gray-500 mt-1">Tambahkan jadwal baru ke sistem</p>
        </div>
    </div>

    <!-- Form Card -->
    <div class="bg-white rounded-xl shadow-sm">
        <form action="{{ route('jadwal.store') }}" method="POST" enctype="multipart/form-data" class="p-6">
            @csrf
                    ...
                    <div>
                        ...
                        <select name="id_tim_sales" 
                            ...
                            @foreach($tim_sales as $item)
                                <option value="{{ $item->id_tim_sales }}" {{ old('id_tim_sales') == $item->id_tim_sales ? 'selected' : '' }}>
                                    {{ $item->nama_tim_sales }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <select name="id_lokasi" 
                        ...
                        @foreach($lokasi as $item)
                            <option value="{{ $item->id_lokasi }}" {{ old('id_lokasi') == $item->id_lokasi ? 'selected' : '' }}>
                                {{ $item->alamat }}
                            </option>
                        @endforeach
                    </select>
