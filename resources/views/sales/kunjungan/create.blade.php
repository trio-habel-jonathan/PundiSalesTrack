...
<div class="container mx-auto p-6">
    ...
    <div class="bg-white rounded-xl shadow-sm">
        <form action="{{ route('sales.kunjungan.store') }}" method="POST" enctype="multipart/form-data" class="p-6">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <!-- Jadwal (Read-Only) -->
                <div class="md:col-span-2">
                    <label for="jadwal_detail" class="block text-sm font-medium text-gray-700 mb-2">
                        Jadwal
                    </label>
                    <input type="text" 
                           id="jadwal_detail" 
                           class="w-full px-4 py-2 rounded-lg border border-gray-200 bg-gray-100"
                           value="{{ $jadwal->tanggal_kunjungan }} - {{ $jadwal->tim_sales->nama_tim_sales }}" 
                           disabled>
                    <input type="hidden" name="id_jadwal" value="{{ $jadwal->id_jadwal }}">
                </div>

                <!-- Sales Profile (Read-Only) -->
                <div class="md:col-span-2">
                    <label for="sales_detail" class="block text-sm font-medium text-gray-700 mb-2">
                        Sales
                    </label>
                    <input type="text" 
                           id="sales_detail" 
                           class="w-full px-4 py-2 rounded-lg border border-gray-200 bg-gray-100"
                           value="{{ $profil_sales->nama }} - {{ $profil_sales->tim_sales->nama_tim_sales }}" 
                           disabled>
                    <input type="hidden" name="id_profile_sales" value="{{ $profil_sales->id_profile_sales }}">
                </div>
